...
import { generatePODOCX } from '../utils/poDocxGenerator'
...
const handlePrint = () => {
  generatePODOCX(formData.value, products.value, selectedCatalog.value)
}
  ...
  <div>
    ...
    <div class="max-w-[8.5in] mx-auto mb-6 flex flex-col gap-4 no-print">
      <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-800">PO Generator</h1>
        <button
          @click="handlePrint"
      ...
      <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex items-center gap-4">
        <label for="po-catalog-select" class="font-semibold text-gray-700">Select Product Catalog:</label>
        <select
          id="po-catalog-select"
    ...
    <div class="doc-container shadow-xl print:shadow-none print:w-full">
      ...
      <div class="border-b-2 border-gray-800 mb-6 pb-2">
        <h1 class="text-3xl font-bold uppercase tracking-widest text-center">Purchase Order</h1>
      </div>

      <POHeader
        v-model:date="formData.date"
        v-model:poNumber="formData.poNumber"
        v-model:supplier="formData.supplier"
        v-model:buyer="formData.buyer"
      />
